...
function login($username, $password, $mysqli){
	...
	if($stmt = $mysqli->prepare("SELECT id, password, level FROM user WHERE username = ?")){
		...
		$stmt->store_result();
		$stmt->bind_result($id, $dbpassword, $leveluser); // Menyimpan nilai hasil query ke variabel
		$stmt->fetch();
