...
<script type="text/javascript">
		...
		function getData(namasup){
			alert('nama suplier = '+namasup);
			objAjax3=buatajax();
			url3="odapatdatasuplier.php";
			url3=url3+"?namasup="+namasup;
		...
		function gantinamasup(){
			...
			if(objAjax3.readyState==4){
				...
				if(data2.length>0){
					document.getElementById("hasilsuplier").value=data2;
				}
				else{
					document.getElementById("hasilsuplier").value="";
				}
			}
		}
...
<body>
	...
	<div id="section">
		...
		<form name="f" method="post" action="";">
			<table>
				...
				<tr>
					...
					<td>:</td>
					<td>
						<select name="NamaSup" onchange="getData(this.value)">
						<?php  
							include 'koneksi.php';
							$query = "SELECT NamaSup FROM supplier";
							$exec = mysql_query($query);
							while($row = mysql_fetch_assoc($exec))
							{
								$NamaSup = $row["NamaSup"];
								echo "<option value='".$row['NamaSup']."'>".$row['NamaSup']."</option>";
							}
						?>
						</select>
					</td>
		...
		<table>
			...
			<tr>
				<td colspan=4 hidden>
					<input type="text" name="hasilsuplier" id="hasilsuplier" readonly="yes">
				</td>
			</tr>
